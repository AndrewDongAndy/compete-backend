...
const userUrl = (userId: string) => {
  return `https://www.acmicpc.net/user/${userId}`;
};
...
export const getUserSolves = async (userId: string): Promise<UserSolves> => {
  const html = await getParsedHtml(userUrl(userId));
  ...
  for (const panel of panels) {
    ...
    if (title == ACCEPTED_HEADING) {
      ...
      console.error("another heading was found on user page:", userId);
    }
  }
  return res;
};
